      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {solutions.map((solution, idx) => (
            <div
              key={idx}
              className="flex items-center p-4 bg-[var(--card)] rounded-lg shadow-md border border-[var(--stroke)]"
            >
              <CheckCircle className="w-6 h-6 text-[var(--brand)] mr-4" aria-hidden="true" />
              <span className="text-lg text-[var(--fg)]">{solution}</span>
            </div>
          ))}
        </div>
      </div>

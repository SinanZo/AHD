    ...
    >
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6 min-h-screen">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-screen items-center">
          ...
          <div className={`text-adh-text ${isRTL ? 'text-right lg:order-2' : 'text-left'}`}>
              <Link 
              to="/products" 
              className="inline-block px-6 py-3 rounded-full text-sm font-semibold tracking-wide uppercase shadow-lg bg-linear-to-r from-teal-600 to-cyan-600 text-adh-text hover:from-teal-500 hover:to-cyan-500 transition-all mb-6"
            >
              {t('hero:badge')}
            </Link>

            <h1 className="hero-title text-adh-text text-[clamp(32px,6vw,72px)] font-bold mb-6 leading-tight">
              {t('hero:title')}
            </h1>

            {t('hero:subtitle') && (
              <p className="text-xl md:text-2xl mb-4 text-adh-text-secondary font-semibold">
                {t('hero:subtitle')}
              </p>
            )}

            <p className="text-base md:text-lg mb-8 leading-relaxed text-adh-text-secondary max-w-2xl">
              {t('hero:description')}
            </p>
          ...
          <div className={`${isRTL ? 'text-right lg:order-1' : 'text-left'} lg:flex lg:items-center lg:justify-end`}>
            <div className="bg-black/40 backdrop-blur-md rounded-2xl p-8 max-w-md border border-adh-stroke">
              <p className="text-sm font-semibold text-cyan-400 uppercase tracking-wider mb-3">
                BLIND MOTORIZATION
              </p>
              <h2 className="text-2xl md:text-3xl font-bold text-adh-text mb-6">
                Smart Control • Instant Comfort
              </h2>
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <span className="text-3xl">📱</span>
                  <span className="text-adh-text font-medium">Mobile App</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-3xl">⏰</span>
                  <span className="text-adh-text font-medium">Schedules</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-3xl">🏠</span>
                  <span className="text-adh-text font-medium">Smart-Home Ready</span>
                </div>
              </div>
            </div>
          </div>

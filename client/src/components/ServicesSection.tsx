...
const CurtainsIcon = ({ className }: { className?: string }) => (
  ...
  >
    <path
      d="M3 2h18v2H3V2zm0 4h4v16H3V6zm6 0h2v16H9V6zm4 0h2v16h-2V6zm4 0h4v16h-4V6z"
      fill="currentColor"
    />
    <path
      d="M5 8c1 2 1 4 0 6M11 8c1 2 1 4 0 6M15 8c1 2 1 4 0 6M19 8c1 2 1 4 0 6"
      stroke="currentColor"
      strokeWidth="1"
    />
  </svg>
...
export default function ServicesSection({
  ...
  const dir =
    ...
    (i18n?.language
      ? i18n.language.slice(0, 2).toLowerCase() === 'ar'
        ? 'rtl'
        : 'ltr'
      : undefined);
  ...
  const preferAr =
    ...
    !!i18n.getResourceBundle('ar', 'services');
  ...
  const catalogRaw = t('catalog', { returnObjects: true });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && process.env.NODE_ENV !== 'production') {
      try {
        ...
        console.debug(
          '[ServicesSection] language=%s services=%o catalog=%o',
          i18n.language,
          services,
          catalog,
        );
      ...
      <>
        {catalog.map((blk, i) => {
          ...
          return (
            <div
              key={`catalog-${i}`}
              className="rounded-2xl overflow-hidden border border-adh-stroke bg-adh-surface/60 backdrop-blur-sm"
            >
              <div className="p-0">
                <h4 className="m-0">
                  <button
                    ...
                    className="w-full text-left px-6 py-4 flex items-center justify-between gap-4 text-adh-text"
                  >
                    <span
                      className={
                        isRTL
                ...
                >
                  <div className="py-4">
                    {blk.desc ? (
                      <p className="text-adh-text-muted mb-3">{blk.desc}</p>
                    ) : null}
                    {Array.isArray(blk.bullets) && blk.bullets.length > 0 && (
                      <ul className="list-inside list-disc space-y-2 text-adh-text">
                        {blk.bullets.map((b: string, idx: number) => (
                          <li key={idx} className={isRTL ? 'font-arabic' : undefined}>
                            {b}
                          </li>
                        ))}
                      </ul>
  ...
  const cards = services.map((svc, i) => {
    ...
    return (
      <Motion.div key={`svc-${i}`} variants={cardVariants} className="w-full">
        <Card className="p-6 h-full bg-adh-surface/70 border border-adh-stroke shadow-adh-soft hover:shadow-adh-card-hover transition-shadow duration-theme">
          <Link
            href={svc.slug || '#'}
            className="no-underline text-adh-text block h-full"
          >
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-adh-chip text-adh-primary shadow-adh-soft border border-adh-stroke/70">
                <Icon className="w-6 h-6" />
              </div>
              <div className="flex-1">
                <h3 className="m-0 text-lg font-semibold text-adh-text">
                  {svc.title}
                </h3>
                {svc.desc ? (
    ...
    <section id={id} className={className} aria-label={title}>
      <div className="container mx-auto py-12">
        <header className="mb-8 max-w-3xl">
          <p className="text-sm uppercase tracking-[0.18em] text-adh-text-muted">
            {subtitle}
          </p>
          <h2 className="text-2xl md:text-3xl font-bold mt-2 text-adh-text">
            {title}
          </h2>
          {description ? (
